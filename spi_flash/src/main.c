...
void single_sector_test(const struct device *flash_dev)
{
	...
	strcat(expected,"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
	...
	while (Idx < (128 * 2580))
	{
		...
		rc = flash_write(flash_dev, SPI_FLASH_TEST_REGION_OFFSET + Idx, expected, len);
		if (rc != 0) {
	...
	{
		memset(buf, 0, len);
		rc = flash_read(flash_dev, SPI_FLASH_TEST_REGION_OFFSET + Idx, buf, len);
		if (rc != 0) {
