...
int main(void)
{
	...
	if (nError) 
    {
     ...
     nvs_initialisation(&fs);                                              //NVS  flash initialisation
    ...
    while (1) 
    {
        ...
        cJsonBuffer = cJSON_Print(pMainObject); //copy json to file
        ...
        if(!IsConnected())              //save to flash only if Mobile Phone is NOT connected
        {
            ...
            if(data_count>= count_max || GetCharaStatus() == true ) 
            {
                ...
                // {
                //  readJsonToFlash(&fs, data_count, count_max, buf, strlen(cJsonBuffer));
                //   printk("\nId: %d, Stored_Data: %s\n",STRING_ID + data_count, buf); 
                // }
