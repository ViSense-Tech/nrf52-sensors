...
static bool ble_data_read = false; 
...
/**
 * @brief File System For Ble 
 * @param attr - pointer to file system 
 * **/
...
void BleHistoryDataNotify(const struct bt_gatt_attr *attr, uint16_t value)
{
    if (value == BT_GATT_CCC_NOTIFY)
    {
		...
		//printk("hchara-attr:%d",attr->uuid);
    }
...
int VisenseSensordataNotify(uint8_t *pucSensorData, uint16_t unLen)
{
    ...
    if (pucSensorData)
    {
	    nRetVal = bt_gatt_notify(NULL, &VisenseService.attrs[1], 
                                pucSensorData, unLen);
		printk("attr count %d\n\r",VisenseService.attr_count);
		if(nRetVal)
		{
			printk("Notification0 failed%d\n\r",nRetVal);
		}
    }
...
void VisenseHistoryDataNotify()                                        //history
{
	bool bRetVal = false;
	uint8_t ucIdx = 1;
	char unLen[90] ;
	...
	// int rc = readJsonToFlash(FileSys, ucIdx, 0, unLen, sizeof(unLen));
 ...
 * @param None
 * @return returns if history notifications is enabled or not.
*/
...
void SetCharaStatus(bool bStatus)
{
	ble_data_read = bStatus;
}